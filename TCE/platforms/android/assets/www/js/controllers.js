...
angular.module('starter.controllers', ['ionic.cloud'])
    ...
    .controller('addMyPetsCtrl', function($scope, $ionicPopup, $timeout, BlankService, $window, $state, $ionicHistory, ImageService, $http  ) {
        ...
        function processAssignAlarm(actuacion, addMinutes) {
            console.log('addMyPetsCtrl -- processAssignAlarm');
              ...
              if ((actuacion != undefined) && (actuacion.date != undefined)) {
                if (actuacion.nameAlarm != undefined) {
                    if (actuacion.alarmId != "0") {
                        ...
                        console.log('addMyPetsCtrl -- processAssignAlarm -- nowDate ', nowDate);
                        //var now = parseDate(nowDate);
                        var now = actuacion.date;
                        console.log('addMyPetsCtrl -- processAssignAlarm -- now', now);
                        var timeAlarm;
                        ...
                        if(addMinutes!=null){
                            if(addMinutes!=undefined){
                                if(addMinutes>0){
                                    console.log('addMyPetsCtrl -- processAssignAlarm -- adding minutes ', addMinutes);
                                    timeAlarm.setTime(timeAlarm.getTime() + addMinutes*120000+60000);
                                    console.log('addMyPetsCtrl -- processAssignAlarm -- hora prevista notificacion ', timeAlarm);
                                }
                            }
                        }
                        ...
                        console.log('addMyPetsCtrl -- asignoAlarmaInSystem with');
                        console.log('addMyPetsCtrl -- text: ', text);
                        console.log('addMyPetsCtrl -- idAlarm: ', idAlarm);
                        console.log('addMyPetsCtrl -- treatmentId: ', treatmentId);
                        console.log('addMyPetsCtrl -- tag: ', tag);
                        console.log('addMyPetsCtrl -- cuando: ', cuando);
                        ...
                        $http(req).success(function(resp){
                            console.log('addMyPetsCtrl -- Notificacion -asignoAlarmaInSystem OK');
                            console.log('addMyPetsCtrl -- Notificacion -text: ', text);
                            console.log('addMyPetsCtrl -- Notificacion -idAlarm: ', idAlarm);
                            console.log('addMyPetsCtrl -- Notificacion -treatmentId: ', treatmentId);
                            console.log('addMyPetsCtrl -- Notificacion -processAssignAlarm-- cuando: ', cuando);
                            console.log("addMyPetsCtrl -- Notificacion -Ionic Push: Push success", resp);
                        }).error(function(error){
                            console.log("addMyPetsCtrl -- Notificacion -Ionic Push: Push error ", error);
                        });
...
*/
        $scope.addImage = function(type, comeFromDetail) {
            console.log("ImagePickerController - addImage type ", type);
            console.log("ImagePickerController - addImage comeFromDetail ", comeFromDetail);
            ImageService.handleMediaDialog(type).then(function(result) {
                console.log("ImagePickerController - addImage then function ", result);
                assignImageToView(result, comeFromDetail)
                $scope.$apply();
            });
        };

        function assignImageToView(msg, comeFromDetail) {
            console.log("ImagePickerController -- assignImageToView ", msg);

            //if ($scope.comeFromDetail) {
            if (comeFromDetail==1) {
                console.log("ImagePickerController -- assignImageToView 1 ");
                if ((BlankService.detailPet != undefined) && (BlankService.detailPet != null) && (BlankService.detailPet.image != null) && (BlankService.detailPet.image != null)) {
                    ...
                    console.log("ImagePickerController -- assignImageToView 2 ");
                }
            } else {
                console.log("ImagePickerController -- assignImageToView 3 ");
                if (($scope.interfaz != undefined) && ($scope.interfaz != null) && ($scope.interfaz.imagePet != null) && ($scope.interfaz.imagePet != null)) {
                   ...
                   console.log("ImagePickerController -- assignImageToView 4 ");
                }
            }
 ...
 console.log('addTreatmentCtrl -- timeAlarm: ', timeAlarm);

                        if(addMinutes!=null){
                            if(addMinutes!=undefined){
                                if(addMinutes>0){
                                    console.log('addTreatmentCtrl -- processAssignAlarm -- adding minutes ', addMinutes);
                                    timeAlarm.setTime(timeAlarm.getTime() + addMinutes*120000+60000);
                                    console.log('addTreatmentCtrl -- processAssignAlarm -- hora prevista notificacion ', timeAlarm);
                                }
                            }
                        }
        ...
        function processAssignAlarm(actuacion, addMinutes) {
            ...
            if ((actuacion != undefined) && (actuacion.date != undefined)) {
                if (actuacion.nameAlarm != undefined) {
                    if (actuacion.alarmId != "0") {
                        var nowDate = actuacion.date;
                         console.log('detailTreatmentCtrl -- nowDate: ', nowDate);
                        //var now = parseDate(nowDate);
                        var now = actuacion.date;
...
/*
 ...
 console.log('detailTreatmentCtrl -- timeAlarm: ', timeAlarm);
...
*/
                        ...
                        //Sobreescribo para que siempre sea 24 horas antes
                        timeAlarm = new Date(now - 24 * 60 * 60 * 1000);
                        console.log('detailTreatmentCtrl -- processAssignAlarm -- hora prevista notificacion ', timeAlarm);
                        ...
                        console.log('detailTreatmentCtrl -- tag: ', tag);
